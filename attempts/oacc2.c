...
void mult_mat_vet(float** matriz, int* offset, float* vetor, float* result, int tdom)
{
	...
	{
		tmp = 0.00;
		if (((i + offset[0]) >= 0) && (matriz[i][0] != 0))
			tmp += matriz[i][0] * vetor[i + offset[0]];
		if (((i + offset[1]) >= 0) && (matriz[i][1] != 0))
			tmp += matriz[i][1] * vetor[i + offset[1]];
		if (((i + offset[2]) >= 0) && (matriz[i][2] != 0))
			tmp += matriz[i][2] * vetor[i + offset[2]];
		if (((i + offset[3]) >= 0) && (matriz[i][3] != 0))
			tmp += matriz[i][3] * vetor[i + offset[3]];
		if (((i + offset[4]) >= 0) && (matriz[i][4] != 0))
			tmp += matriz[i][4] * vetor[i + offset[4]];
		result[i] = tmp;
	}
}
